...
> [!TIP] Använd filter (Data → Skapa ett filter) för att snabbt hitta frågetecken, NA eller dubbletter.
...
<textarea id="log-template" hidden>
## Laborationslogg – Energioptimering
### Scenario
- Fil: ________
- Kort beskrivning: ________
- Datum: ________

### Datadiagnos
| Problem | Rad/kolumn | Förklaring | Beslut | Motivering |
| --- | --- | --- | --- | --- |
| | | | | |
| | | | | |

### Datastädning
- Åtgärdade saknade värden: ________
- Enhetskonverteringar: ________
- Dubbletter/avvikare: ________
- Frågor till uppdragsgivaren: ________

### Modellresultat
- Lutning (°C → kWh): ________
- Skärning (intercept): ________
- R² / Egen bedömning: ________
- Kommentar om kvalitet: ________

### Nästa steg
- Vad behöver förbättras?
- Ytterligare data eller features?
</textarea>
...
<button
  ...
  type="button"
  onclick="(async(btn)=>{const label=document.getElementById('copy-log-template-label');const defaultText=btn.dataset.defaultText||'Kopiera loggmall';const source=document.getElementById('log-template');if(!source||!navigator.clipboard)return;try{await navigator.clipboard.writeText((source.value||'').trim());if(label){label.textContent='Mall kopierad!';setTimeout(()=>{label.textContent=defaultText;},2000);}}catch(error){console.error('Kopiering misslyckades',error);if(label){label.textContent='Kunde inte kopiera';setTimeout(()=>{label.textContent=defaultText;},2000);}}})(this)"
>
...
- Alternativt (om du vill använda Python) exporterar du den städade datan via **Arkiv → Ladda ned → Kommaavgränsade värden (.csv)** och tränar med `LinearRegression`.
